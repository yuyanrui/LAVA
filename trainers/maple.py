...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        if self.prompt_learner.training:
            if self.cfg.DATASET.NUM_CLASSES == 1:
                label = label.to(logits.dtype).view(-1, 1)
    ...
    def reid_similarity(self, image1, image2):
        
        prompts, shared_ctx, deep_compound_prompts_text, deep_compound_prompts_vision = self.prompt_learner()
        
        image_features1 = self.image_encoder(image1.unsqueeze(0).to(self.dtype), shared_ctx, deep_compound_prompts_vision)
        image_features2 = self.image_encoder(image2.unsqueeze(0).to(self.dtype), shared_ctx, deep_compound_prompts_vision) 
...
class MaPLe(TrainerX):
    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            # dataloader都需要什么？修改dataloader的代码到我的需求，classnames是从哪里传过来的？
